...
```
```{r Fig1, echo=TRUE, comment=NA, results='asis', message=FALSE, fig.align='center', fig=TRUE}
...
**Your Answers here:**
<br>1. 6 components (cumulative % variance explained is 52%)

<br>2. 5yr and 10yr Treasury notes have highest weights of 0.93 because the US Federal Bank has a large influence on the global economy.

<br>3. Component #1 = bond futures, #2 = FX futures, #3 = equity index futures, #4 = North American equity index futures, #5 = benchmark interest rates, #6 = oil futures #7 = metals futures, #8 = agricultural futures, #9 = precious metals, #10 = asian equities indices

<br>4. a) pre-crisis bull market years (e.g. only using the data between November 1, 2002, and October 1, 2007)
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
precrisis_data = futures_data[as.Date(rownames(futures_data)) > "2002-11-01" & as.Date(rownames(futures_data)) < "2007-10-01", ]

Variance_Explained_Table_results_precrisis <- PCA(precrisis_data, graph=FALSE)
Variance_Explained_Table_precrisis <- cbind(paste("component",1:ncol(precrisis_data),sep=" "),Variance_Explained_Table_results_precrisis$eig)
Variance_Explained_Table_precrisis <- as.data.frame(Variance_Explained_Table_precrisis)
colnames(Variance_Explained_Table_precrisis) <- c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")

eigenvalues_precrisis  <- Variance_Explained_Table_precrisis[,2]
```

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data_precrisis = data.frame(Variance_Explained_Table_precrisis)
m1_precrisis <-gvisTable(show_data_precrisis,options=list(width=1920, height=min(400,27*(nrow(show_data_precrisis)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1_precrisis,'chart')
```

```{r Fig2, echo=FALSE, comment=NA, results='asis', message=FALSE, fig.align='center', fig=TRUE}
df_precrisis           <- cbind(as.data.frame(eigenvalues_precrisis), c(1:length(eigenvalues_precrisis)), rep(1, length(eigenvalues_precrisis)))
colnames(df_precrisis) <- c("eigenvalues", "components", "abline")
Line_precrisis         <- gvisLineChart(as.data.frame(df_precrisis), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues_precrisis'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Line_precrisis, 'chart')
```


```{r echo=TRUE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis',tidy=TRUE}
#generic code for producing the component weightings, where "my_data" should be changed
my_data = precrisis_data
corused = cor(my_data[,apply(my_data!=0,2,sum) > 10, drop=F])
Rotated_Results <- principal(corused, nfactors=20, rotate="varimax",score=TRUE)
Rotated_Factors <- round(Rotated_Results$loadings,2)
Rotated_Factors <- as.data.frame(unclass(Rotated_Factors))
colnames(Rotated_Factors) <- paste("Component",1:ncol(Rotated_Factors),sep=" ")
...
<br>b) the financial crisis years (e.g. only using the data between October 1, 2007 and March 1, 2009)
Commodities became much more correlated with the overall futures market during this period, where metals and oil were more relevant pre-crisis.
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
...
Line_crisis         <- gvisLineChart(as.data.frame(df_crisis), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues_crisis'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
...
<br>
<br>5. It takes less components to explain 50% of the variance during the crisis years, suggesting that during a crisis, market sentiment is likely swayed by a limited number of key commodities that signal to the market the overall outlook. Specifically we find that US Treasury rates are most influential during crisis years, suggesting that the policies of the Federal Reserve Bank have a significant influence on market returns during market turmoil.
<br>
...
**Your Answers here:**
<br>1. The code takes the moving Average (MA) Signals for each of the 64 futures and let them 'vote' with their sign (+-1, e.g. long or short vote, for each signal). Then we invest $1 every day (so the sum of the absolute values of the weights across the 64 futures totals to 1 every day). Then we create the returns of the strategy for each futures time series and use the signal from 2 days ago.
<br>
<br>2. 
<br>
Adjusting the strategy using "MAfreq<-seq(10,500,by=20) " instead.

```{r echo=FALSE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
signal_used = 0*futures_data # just initialize the trading signal to be 0
# Take  moving Average (MA) Signals, let them "vote" with their sign (+-1, e.g. long or short)
MAfreq<-seq(10,500,by=20)
for (iter in 1:length(MAfreq))
  signal_used = signal_used + sign(apply(futures_data,2, function(r) ma(r,MAfreq[iter])))
# Now make sure we invest $1 every day 
signal_used = t(apply(signal_used,1,function(r) {
  res = r  
  if ( sum(abs(r)) !=0 )
    res = r/sum(abs(r))
...
This is the performance of the new strategy:
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=5,fig.width= 8, fig=TRUE}
pnl_plot(trading_strategy)
```
<br>
