...
**Your Answers here:**
<br>1. Apple stock returns are downloaded using the "dataSet1.R" file, and the code for this is:
<br>mytickers = c("SPY", "AAPL")
<br>the plot of Apple stock is:
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=4,fig.width= 6, fig=TRUE}
AAPL = StockReturns[,"AAPL"]
pnl_plot(AAPL)
```
<br>
<br>2. The cumulative returns of the Apple stock during this period is `r round(100*sum(AAPL),1)`%.
<br>The average daily returns of the Apple stock during this period is `r round(100*mean(AAPL),3)`%;
<br>The standard deviation of the daily returns of the Apple stock during this period is `r round(100*sd(AAPL),3)`%;
<br>
<br>3. the plot of Yahoo stock is:
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=4,fig.width= 6, fig=TRUE}
YHOO = StockReturns[,"YHOO"]
pnl_plot(YHOO)
```
<br>The cumulative returns of the Yahoo stock during this period is `r round(100*sum(YHOO),1)`%.
<br>The average daily returns of the stock during this period is `r round(100*mean(YHOO),3)`%;
<br>The standard deviation of the daily returns of the stock during this period is `r round(100*sd(YHOO),3)`%;
...
**Your Answers here:**
<br>1. The code in "dataSet1.R" file, line 9, needs to be changed to "startDate = "2001-01-01""
<br>After changing that code, we need to run the command "source("dataSet1.R")" again to download the new data.
<br>
<br>
...
**Your Answers here:**
<br>1.  The answer is 20.
<br>
<br>2. There will be an error when trying to add "myData" and the first 40 data for "StockReturns" because myData only containes 20 days worth of data.
<br>
