...
##### Answers:
<br>1. Groups of questions with high correlation are brand and status, self-reward and status, achievement and self-reward. This makes sense because we would expect buying a well known brand is a way to express social status, and the satisfaction of being able to demonstrate status correlates with the idea that buying a boat is a reward for oneself, and the purchase is done generally when the customer has a sense of self-achievement which drives the desire to reward oneself.
<br>
<br>2. These correlations imply that the purchase choices are not necessarily being driven by a concern for the specific product features or the product performance, but more for the satisfaction gained by buying something that signals status and wealth to others.
<br>
...
**Your Answers here:**
<br>1. The table shows the "components" i.e. different combinations of the 29 "attitudes" related data, and their effectiveness in explaining the variance in the data, which is expressed as the % of explained variance in column 3. We can learn that component 1 alone can explain nearly 30% of the variance in the data. The Scree plot shows the percentages graphically. 

<br>2. The plot is a decreasing line because the PCA analysis is done (by definition) so that the components are generated in order of decreasing eigenvalue.

<br>3. What we want to really know is the breakdown of the attitudes for each component are, and their respective weights within the component. That way we can better understand the meaning of each component, e.g. maybe Component 1 is a group of attitudes related to brand. 
<br>
...
**Your Answers here:**
<br>1. The first factor (Component 1) is tied to social status and boating as a passion. The mix of these two seems a bit random. Component 2 focuses around the emotional experience of boating and other positive psychological benefits and seems like a natural grouping.
<br>2. Maybe 3-5 factors is enough, otherwise the incremental explanatory power of more factors is below 3% and seems insignificant.
<br>3. If we choose the first 4 factors, # 1 and #2 are as explained above. #3 expresses the importance of a high brand or good technology, and #4 shows the level of boating expertise.
<br>4. Given the overlaps in component groupings, this analysis shows that a certain level of discretion an interpretation is needed when using the results of statistical tools in order to gain meaningful insights.
...
**Your Answers here:**
<br>1. There are 3 segments
<br>2. Segment 3 are boat enthusiasts who prioritize specs. Segment 1 are least knowledgable about boats and not passionate about boating itself. Segment 2 are boat owners who are passionate but maybe don't care too much about specs and brand as much as Segment 3.
<br> 3. If we can reduce the number of factors, it will be easier to understand how the segments are different from each other. The defining characteristics will be more apparent than when we try to look at 29 attitudes one by one.
<br>4. 
